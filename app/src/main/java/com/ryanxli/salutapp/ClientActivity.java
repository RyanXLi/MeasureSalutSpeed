...
public class ClientActivity extends AppCompatActivity implements SalutDataCallback, View.OnClickListener {
    ...
    public ArrayList<Long> speedArrayList;
    public ArrayList<String> deviceArrayList;

    public static final int trials = 200;

    public static int trialSize = 1024 * 2; // in Kilobits
//    public static int waitTime = 1000 * 4; //1000*trialSize / 1024 / 3 + 1;// mill
...
//        }
        ...
        try
        {
            ...
            speedArrayList.add((ClientActivity.trialSize * 1000)/(newMessage.receiveTime - newMessage.sendTime));
            // in kbps
...
//            });
        ...
        for (long i : speedArrayList) { sum+=i; }
        mean = sum / speedArrayList.size();

        for (long i : speedArrayList) {
            num += Math.pow((double) i - mean, 2);
        }
        sd = Math.sqrt(num/speedArrayList.size());

        messageArrayList.add("mean: " + mean + " kbps");
        messageArrayList.add("sd: " + sd + " kbps");
